...
from automl.utils import Timer, touch
...
def run(dataset: Dataset, config: TaskConfig):
    ...
    try:
        ...
        h2o.init(nthreads=nthreads,
                 ...
                 # log_dir=make_subdir('logs', config),
                 )
...
def save_artifacts(automl, dataset, config):
    save_logs = config.framework_params.get('_save_logs', False)
    save_models = config.framework_params.get('_save_models', False)
    save_preds = config.framework_params.get('_save_predictions', False)
    try:
        ...
        if save_models:
            ...
            if save_models != "leaderboard":
                all_models_se = next((mid for mid in lb['model_id'] if mid.startswith("StackedEnsemble_AllModels")),
                                     None)
                if all_models_se:
                    save_model(all_models_se, dest_dir=models_dir, mformat=save_models)
                else:
                    for mid in lb['model_id']:
                        save_model(mid, dest_dir=models_dir, mformat=save_models)
